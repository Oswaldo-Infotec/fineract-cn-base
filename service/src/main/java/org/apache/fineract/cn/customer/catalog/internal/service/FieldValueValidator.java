...
public class FieldValueValidator {
  ...
  public void validateCURP(final String curp) {
    String PATTERN = "[A-Z]{1}[AEIOUX]{1}[A-Z]{2}[0-9]{2}"
      ...
      } else if(!curp.toUpperCase().matches(PATTERN)) {
  ...
  public void validateRFC(final String rfc){
    String PATTERN = "[A-Z]{0,4}[0-9]{2}(0[1-9]|1[0-2])(0[1-9]|1[0-9]|2[0-9]|3[0-1])[A-Z0-9]{3}";
    if(rfc == null || rfc.equals("")){
      throw ServiceException.conflict("The federal taxpayers registry (RFC) {0} is a required field.", rfc);
    } else if(!rfc.toUpperCase().matches(PATTERN)) {
      throw ServiceException.conflict("The federal taxpayers registry (RFC) {0} is not valid.", rfc);
    }
  }
